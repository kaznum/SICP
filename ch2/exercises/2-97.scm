...
(define (install-polynomial-package)
  ...
  (define (gcd-coeffs L1)
    (define (reduction-coeffs d L1)
      (if (null? L1)
	  (the-empty-termlist)
	  (adjoin-term (make-term (order (first-term L1)) (/ (coeff (first-term L1)) d))
		       (reduction-coeffs d (rest-terms L1)))))
    (let ((d (apply gcd (map coeff L1))))
      (reduction-coeffs d L1)))
  ...
  (define (pseudoremainder-terms a b)
    (let ((t1 (first-term a))
      ...
      (let ((o1 (order t1))
	    ...
	    (c2 (coeff t2)))
	(cond ((< o1 o2)
	       (cadr (div-terms a (mul-terms (list (make-term 0 (expt c1 (+ 1 (- o2 o1))))) b))))
	      ((= o1 o2)
	       (cadr (div-terms a b)))
	      (else
	       (cadr (div-terms (mul-terms (list (make-term 0 (expt c2 (+ 1 (- o1 o2))))) a) b)))))))
  ...
  ;; The answer of ex2.97 (a)
  (define (reduce-terms n d)
    (let ((gcd-of-n-and-d (gcd-terms n d)))
      (let ((nn (car (div-terms n gcd-of-n-and-d)))
	    (dd (car (div-terms d gcd-of-n-and-d))))
	(list nn dd))))
  ...
  (define (reduce-poly p1 p2)
    (let ((var1 (variable p1))
	  (var2 (variable p2)))
      (if (eq? var1 var2)
	  (let ((reduced-polies (reduce-terms (term-list p1)
					      (term-list p2))))
	    (list (make-poly var1 (car reduced-polies))
		  (make-poly var1 (cadr reduced-polies))))
  ...
  (put 'reduce '(polynomial polynomial)
       (lambda (p1 p2)
	 (let ((reduced-n-d (reduce-poly p1 p2)))
	   (list (tag (car reduced-n-d))
		 (tag (cadr reduced-n-d))))))
  'done)
...
(define (reduce p1 p2) (apply-generic 'reduce p1 p2))
...
(define p11 (make-polynomial 'x '((1 1)(0 1))))
(define p12 (make-polynomial 'x '((3 1)(0 -1))))
(define p13 (make-polynomial 'x '((1 1))))
(define p14 (make-polynomial 'x '((2 1)(0 -1))))

(define rf11 (make-rational p11 p12))
(define rf12 (make-rational p13 p14))
